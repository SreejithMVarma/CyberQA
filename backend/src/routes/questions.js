...
router.get('/', async (req, res) => {
  try {
    ...
    if (solved === 'unsolved') {
      const answeredQuestions = await Answer.find({ status: 'verified' }).distinct('questionId');
...
router.post('/upload-image', isAuthenticated, isAdmin, upload.single('image'), async (req, res) => {
  try {
    ...
    const imageUrl = `${process.env.STORAGE_PATH}/${req.file.filename}`;
    res.json({ imageUrl });
  } catch (err) {
...
router.post('/', isAuthenticated, isAdmin, async (req, res) => {
  try {
    ...
    if (!questionText || !type || !difficulty) {
      return res.status(400).json({ message: 'Question text, type, and difficulty are required' });
    }
